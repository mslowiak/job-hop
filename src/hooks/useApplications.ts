...
import type { ApplicationDto, ApplicationStatus, ApplicationListResponseDto, ApplicationViewModel } from "../types";
...
export const useApplications = (filter: ApplicationStatus | "all" = "all") => {
  ...
  const [currentFilter, setCurrentFilter] = useState<ApplicationStatus | "all">(filter);
   ...
   */
  const fetchApplications = useCallback(async (filterParam: ApplicationStatus | "all" = "all") => {
    setLoading(true);
    setError(null);
  ...
  const updateStatus = useCallback(
    async (id: string, newStatus: ApplicationStatus): Promise<void> => {
      ...
      try {
        ...
        setApplications((prev) => prev.map((app) => (app.id === id ? { ...app, status: originalStatus } : app)));
